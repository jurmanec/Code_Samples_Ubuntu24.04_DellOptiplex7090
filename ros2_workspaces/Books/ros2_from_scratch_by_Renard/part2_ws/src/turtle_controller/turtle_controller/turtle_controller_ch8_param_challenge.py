...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        # params
        self.declare_parameter("color1", [255,0,0]) # red, green, blue: right side color, pass from command lines as "[1, 2, 3,]"
        self.declare_parameter("color2", [0,255,0]) # red, green, blue: left side color
    ...
    def callback_pose(self, pose: Pose):
        if self.is_active_:
            cmd = Twist()
            # use turtle_velocity and turtle_velocity * 2 instead of 1.0 and 2.0 (turtle_velocity is param)
            if pose.x < 5.5:
                cmd.linear.x = self.get_parameter("turtle_velocity").value
                cmd.angular.z = self.get_parameter("turtle_velocity").value
            else:
                cmd.linear.x = self.get_parameter("turtle_velocity").value * 2
                cmd.angular.z = self.get_parameter("turtle_velocity").value * 2
            self.cmd_vel_pub_.publish(cmd)

            if pose.x > 5.5 and self.previous_x_ <= 5.5:
                self.previous_x_ = pose.x
                color1 = self.get_parameter("color1").value
                self.call_set_pen(*color1)
            elif pose.x <= 5.5 and self.previous_x_ > 5.5:
                self.previous_x_ = pose.x
                color2 = self.get_parameter("color2").value
                self.call_set_pen(*color2)
